...
def object_recognition(imageName, actual_size):
    ...
    total_people = calculate_total_nr_of_people(detected_objects)

    return [nr_of_people, total_people]


def calculate_total_nr_of_people(detected_objects):
    nr = 0
    for key in detected_objects:
        if key == 'person':
            return len(detected_objects[key])
        ...
        for point2 in list_of_points:
            ...
            if x1 != x2 and y1 != y2:
                ...
                for points in distance:
                    p1, p2 = points
                    if (p1 == point2 and p2 == point1) or (p1 == point1 and p2 == point2):
                        ok = 1
                if ok == 0:
...
def number_of_persons_with_less_than_2m(distance_between):
    ...
    for elem in distance_between:
        ...
        if distance_between[elem] < 2:
            if p1 not in list_of_persons:
                count = count + 1
                list_of_persons.append(p1)
            if p2 not in list_of_persons:
                count = count + 1
